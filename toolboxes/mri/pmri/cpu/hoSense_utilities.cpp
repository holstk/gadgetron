...
namespace Gadgetron{
    ...
    mult_csm_kernel( const complext<REAL> * __restrict__ in, //Used in csm_mult_M below
		     complext<REAL> * __restrict__ out, 
		     complext<REAL> *csm,
		     size_t image_elements, 
		     unsigned int nframes, 
		     unsigned int ncoils )
    {
	unsigned int idx = blockIdx.x*blockDim.x+threadIdx.x; //image_element
	...
	mult_csm_kernel<REAL><<< gridDim, blockDim >>> //mult_csm_kernel
    ...
    template <class REAL> __global__ void 
    mult_csm_conj_sum_kernel(const  complext<REAL> * __restrict__ in, //Used in csm_mult_MH below
			     complext<REAL> * __restrict__ out, 
			     const complext<REAL> * __restrict__ csm,
			     size_t image_elements, 
			     unsigned int nframes, 
			     unsigned int ncoils )
    {
	unsigned int idx = blockIdx.x*blockDim.x+threadIdx.x; //Image elements
	if( idx < image_elements ) {
	...
	mult_csm_conj_sum_kernel<REAL><<< gridDim, blockDim >>> //mult_csm_conj_sum_kernel<
	    ( in->get_data_ptr(), out->get_data_ptr(), csm->get_data_ptr(), num_image_elements, num_frames, csm->get_size(D) );
