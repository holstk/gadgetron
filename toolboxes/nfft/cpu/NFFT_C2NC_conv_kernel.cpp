...
template<unsigned int D> static void resolve_wrap( vector_td<int,D> &grid_position, vector_td<unsigned int,D> &matrix_size_os )
{
...
template<class T, unsigned int D> void NFFT_iterate_body( typename reald<T,D>::Type alpha, 
							  typename reald<T,D>::Type beta, 
							  T W, 
							  ...
							  T half_W, 
							  T one_over_W, 
							  vector_td<T,D> matrix_size_os_real, 
							  //unsigned int sharedMemFirstSampleIdx,
							  vector_td<T,D> sample_position, 
							  vector_td<int,D> grid_position,
							  int number_of_frames, int number_of_samples, int idx, bool accumulate )
{
    ...
    const vector_td<T,D> half_W_vec(half_W );
    ...
    const T weight = kb.KaiserBessel( delta, matrix_size_os_real, one_over_W, beta );
    ...
    for( unsigned int coil=0; coil<number_of_coils; coil++ ){
    
	// Read the grid cell value from global memory
	const complext<T> grid_value = 
	    image[ (coil*number_of_frames) * prod(matrix_size_os) + co_to_idx<D>( vector_td<unsigned int, D>(grid_position), matrix_size_os ) ];
    

	unsigned int out_idx = (coil*number_of_frames)*number_of_samples + idx;
...
template<class T> void NFFT_iterate(typename reald<T,1>::Type alpha, 
				    typename reald<T,1>::Type beta, 
				    T W, 
				    vector_td<unsigned int,1> matrix_size_os, 
				    unsigned int number_of_coils, 
				    complext<T> *image,
				    complext<T> *samples,
				    //unsigned int double_warp_size_power, 
				    T half_W, 
				    T one_over_W, 
				    vector_td<T,1> matrix_size_os_real, 
				    //unsigned int sharedMemFirstSampleIdx,
				    vector_td<T,1> sample_position, 
				    vector_td<int,1> lower_limit, 
				    vector_td<int,1> upper_limit,
				    int number_of_frames, int number_of_samples,int idx, bool accumulate  
				    )
{
	...
	vector_td<int,1> grid_position(x);

	NFFT_iterate_body<T,1>(alpha, beta, W, matrix_size_os, number_of_coils, image, samples, 
			       half_W, one_over_W, matrix_size_os_real, sample_position, grid_position, number_of_frames, number_of_samples, idx, accumulate );
...
template<class T> void NFFT_iterate( typename reald<T,2>::Type alpha, typename reald<T,2>::Type beta, T W, 
				     vector_td<unsigned int,2> matrix_size_os, 
				     unsigned int number_of_coils, complext<T> *image,
				     complext<T> *samples,
				     //unsigned int double_warp_size_power, 
				     T half_W, T one_over_W, vector_td<T,2> matrix_size_os_real, 
				     //unsigned int sharedMemFirstSampleIdx,
				     vector_td<T,2> sample_position, vector_td<int,2> lower_limit, vector_td<int,2> upper_limit,
				     int number_of_frames, int number_of_samples, int idx, bool accumulate )
{
    // Iterate through all grid cells influencing the corresponding sample
	...
	for( int x = lower_limit.vec[0]; x<=upper_limit.vec[0]; x++ ){
	    ...
	    NFFT_iterate_body<T,2>( alpha, beta, W, matrix_size_os, number_of_coils, image, samples, half_W, 
				    one_over_W, matrix_size_os_real, sample_position, grid_position, number_of_frames, number_of_samples, idx, accumulate );
	}
    }
}
...
template<class T> void NFFT_iterate( typename reald<T,3>::Type alpha, typename reald<T,3>::Type beta, T W, 
				     vector_td<unsigned int,3> matrix_size_os, unsigned int number_of_coils, complext<T> *image,
				     complext<T> *samples,
				     T half_W, T one_over_W, vector_td<T,3> matrix_size_os_real,
				     vector_td<T,3> sample_position, vector_td<int,3> lower_limit, vector_td<int,3> upper_limit,
				     int number_of_frames, int number_of_samples, int idx, bool accumulate )
{
    // Iterate through all grid cells influencing the corresponding sample
	...
	for( int y = lower_limit.vec[1]; y<=upper_limit.vec[1]; y++ ){
		...
		NFFT_iterate_body<T,3>( alpha, beta, W, matrix_size_os, number_of_coils, image, samples, half_W, 
					one_over_W, matrix_size_os_real, sample_position, grid_position, number_of_frames, number_of_samples, idx, accumulate );
	    }
	}
    }
...
template<class T> void NFFT_iterate( typename reald<T,4>::Type alpha, typename reald<T,4>::Type beta, T W, 
				     vector_td<unsigned int,4> matrix_size_os, unsigned int number_of_coils, complext<T> *image,
				     complext<T> *samples,
				     T half_W, T one_over_W, vector_td<T,4> matrix_size_os_real,
				     vector_td<T,4> sample_position, vector_td<int,4> lower_limit, vector_td<int,4> upper_limit,
				     int number_of_frames, int number_of_samples, int idx, bool accumulate )
{
  // Iterate through all grid cells influencing the corresponding sample
  for( int w = lower_limit.vec[3]; w<=upper_limit.vec[3]; w++ ){
	...
	for( int x = lower_limit.vec[0]; x<=upper_limit.vec[0]; x++ ){
	  ...
	  NFFT_iterate_body<T,4>( alpha, beta, W, matrix_size_os, number_of_coils, image, samples, half_W, 
				  one_over_W, matrix_size_os_real, sample_position, grid_position, number_of_frames, number_of_samples, idx, accumulate );
	}
      }
    }
...
NFFT_convolve_kernel( typename reald<REAL,D>::Type alpha, 
		      typename reald<REAL,D>::Type beta, 
		      REAL W, 
		      vector_td<unsigned int, D> matrix_size_os, 
		      vector_td<unsigned int, D> matrix_size_wrap,
		      ...
		      const vector_td<REAL,D> * __restrict__ traj_positions, 
		      complext<REAL> *image,  
		      complext<REAL> * __restrict__ samples,
		      unsigned int double_warp_size_power, 
		      REAL half_W, 
		      REAL one_over_W, 
		      bool accumulate, 
		      vector_td<REAL,D> matrix_size_os_real )
{
    ...
    NFFT_convolve<REAL,D>( alpha, 
			   beta, 
			   W, 
			   matrix_size_os, 
			   matrix_size_wrap, 
			   number_of_samples, 
			   number_of_batches, //num_coils
			   traj_positions, 
			   image, 
    ...
    NFFT_output<REAL>( number_of_samples, 
		       number_of_batches, 
		       samples, 
		       double_warp_size_power, 
		       globalThreadId, 
