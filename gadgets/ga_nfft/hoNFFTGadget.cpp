...
hoNFFTGadget::hoNFFTGadget()
  : image_counter_(0)
{
}


int hoNFFTGadget::process( GadgetContainerMessage<IsmrmrdReconData>* m1)
{
    std::cout << "\nIN HONFFTGENERAL!\n" << std::endl;
	...
	//constexpr unsigned int D = addr(Ndims);
	//constexpr unsigned int D = foo(Ndims);
	//int* tmp_dims = &Ndims;
	//constexpr int D = const_cast<int&>(*tmp_dims);
	//constexpr unsigned int D = new constexpr(Ndims);
	//constexpr unsigned int D = foo(Ndims);
	//constexpr unsigned int D = dynamic_cast<constexpr unsigned int>(Ndims);

	imarray.data_.create(im_dims[0], im_dims[1], im_dims[2], data_dims[3], data_dims[4], data_dims[5], data_dims[6]);

	//std::cout << "\nE0: " << E0 << "\nE1: " << E1 << "\nE2: " << E2 << "\nCHA: " << CHA << "\nN: " << N << "\nS: " << S << "\nLOC: " << LOC << std::endl;
	//std::cout << "\nim_dims[0]: " << im_dims[0] << "\nim_dims[1]: " << im_dims[1] << "\nim_dims[2]: " << im_dims[2] << std::endl;
	//std::cout << "\ngrid_dims[0]: " << grid_dims[0] << "\ngrid_dims[1]: " << grid_dims[1] << "\nim_dims[2]: " << grid_dims[2] << std::endl;
	...
	*/
        ...
        for (uint16_t loc=0; loc < LOC; loc++) {
		    ...
		    //GDEBUG("Before all header definitions.\n");
                    ...
                    chunk_dims[2] = E2;
                    chunk_dims[3] = CHA;
                    hoNDArray<complext<float> > chunk = hoNDArray<complext<float> >(chunk_dims, &dbuff.data_(0,0,0,0,n,s,loc));
		    ...
		    //samples = new hoNDArray< complext<float> > (&sample_dims);
		    //samples(chunk);
		    ...
		    */
		    ...
		    GDEBUG("Root-SS...\n");
                    ...
                    hoNDArray<complext<float> > output = hoNDArray<complext<float> >(img_dims, &imarray.data_(0,0,0,0,n,s,loc));
		    ...
		    GDEBUG("Output cleared\n");
                    ...
                    multiplyConj(image, image, image);    
			...
			if (Ndims == 2)
			    output += hoNDArray<complext<float> >(img_dims, &image(0,0,c));
			else
			    output += hoNDArray<complext<float> >(img_dims, &image(0,0,0,c));

			if (c==CHA-1)
			    //* WRITE */ write_nd_array<std::complex<float>>(&output, "/home/karen/data/images/output_last_cha.cplx");
			if (c==CHA-1){
			    hoNDArray<complext<float> > im_tmp_first_cha = hoNDArray<complext<float> >(img_dims, &image(0,0,0,c));
...
GADGET_FACTORY_DECLARE(hoNFFTGadget)
}
