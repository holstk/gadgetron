...
typedef reald<_real,3>::Type _reald2;
...
int main( int argc, char** argv) 
{
    ...
    int Ndims = (int)num_dim;
    GATrajectory<float,num_dim> traj;
    hoNDArray<typename reald<float,num_dim>::Type> trajectory;
    ...
    if( !(csm->get_number_of_dimensions() == 4 )){
    ...
    uint64d3 matrix_size = uint64d3(csm->get_size(0), csm->get_size(0), csm->get_size(0));
    size_t _matrix_size_os = size_t((float)matrix_size[0]*parms.get_parameter('a')->get_float_value());
    uint64d3 matrix_size_os = uint64d3(_matrix_size_os, _matrix_size_os, _matrix_size_os);
    ...
    unsigned int num_coils = csm->get_size(num_dim-1+1); //D = 3, csm dims is 0:3 where 0:2 are im dims and 3 is coil dim
    ...
    boost::shared_ptr< hoNonCartesianSenseOperator<_real,num_dim> > E( new hoNonCartesianSenseOperator<_real,num_dim>() );  
    ...
    boost::shared_ptr< hoNDArray<_real> > _precon_weights = sum(abs_square(csm.get()).get(), num_dim-1+1);
