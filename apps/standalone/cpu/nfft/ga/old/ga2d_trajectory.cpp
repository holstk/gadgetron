...
#include "/home/holstk/code/gadgetron/toolboxes/core/cpu/math/hoNDArray_elemwise.h"
#include "hoNDFFT.h"
...
class ga2dTraj {
  ...
  int calculateDCF(hoNDArray<float>& DCF, int Nspokes, int NsamplesPerSpoke, int Ndim, bool cut_off, int grid_size){
    ...
    float DCFstep = 2/(float(NsamplesPerSpoke) - 1); std::cout << "DCFstep = " << DCFstep << std::endl;
    ...
    std::cout << "baseDCF[0] = " << baseDCF[0] << "\nbaseDCF[NsamplesPerSpoke/2] = " << baseDCF[NsamplesPerSpoke/2] <<"\nbaseDCF[NsamplesPerSpoke-1] = " << baseDCF[NsamplesPerSpoke-1] << std::endl;
    ...
    for (int sp = 0 ; sp < Nspokes ; sp++)
    {
      ...
      {
	if (Ndim == 2){
	  DCF[sp * NsamplesPerSpoke + sa] = baseDCF[sa];
	}
	else {
	  DCF[sp * NsamplesPerSpoke + sa] = baseDCF[sa] * baseDCF[sa];
	}
      }
    }
  ...
  int apodization(float sigma, hoNDArray< std::complex<float> >& W){
